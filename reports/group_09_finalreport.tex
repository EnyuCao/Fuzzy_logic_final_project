...
% TODO yellow marked areas from feedback 2
...
% TODO Do not just summarize papers, also provide a critical note, or what you improve compared to their approach.
% Do not subsection the references, just give brief take-away points of the previous work, do not summarise it.
...
% TODO What do you take away from the papers? Do you improve upon them? This was also mentioned in the first milestone, please look back.
...
%TODO Better explanation of the rules
...
% TODO Good that you give results. They are not very clear though. Try to run multiple experiments and show your results more clearly, in tables or figures.
%\subsection{Results}
...
% TODO Now how do we evaluate whether the system is doing good or not? And how does performance improve as we change parameter settings?
...
\section{Discussion}
Judging from our results, there were two visible problems with the fls.

The first being that, when the speed of the robot increases, the duration of the simulations decreases (the robot gets stuck very early on). This can be explained by the fact that the mfs use a distance in pixels to calculate the closeness of objects. The membership functions don't change based on the speed. The fls also does not have speed as one of it's inputs. This means that the fls has no way to correct for speed. When the robot moves at a higher speed the something that was 'far' for a lower speed might now be reached in one frame.
